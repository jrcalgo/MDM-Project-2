...
def read_input_rows(path: str) -> List[MDMData]:
    """
    Reads either a CSV or Excel (XLSX/XLS) file and converts each row into MDMData.
    The function auto-detects the file type based on extension.
    """
    ...
    if path_str.endswith(".csv"):
        ...
        raise ValueError(f"Unsupported file format: {path}. Only CSV and XLSX/XLS are supported.")

    rows: List[MDMData] = []

    # Normalize missing values, ensure strings
    df = df.fillna("")
