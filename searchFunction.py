...
def _drop_unprovenanced_fields(out: Dict[str, Any], provenance: Any) -> Tuple[Dict[str, Any], List[str]]:
    ...
    # confidence we leave as-is (just a score), but can drop if desired:
    # if not _has_prov("confidence"): ...
...
def main():
    in_path = os.getenv("MDM_INPUT_XLSX", "query_group2.xlsx")
    out_path = os.getenv("MDM_OUT_JSON", "mdm_results.json")
    max_rows_env = os.getenv("MDM_MAX_ROWS")
    max_rows = int(max_rows_env) if (max_rows_env and max_rows_env.isdigit()) else None

    rows = read_input_rows(in_path)
    if max_rows is not None:
        rows = rows[:max_rows]
    print(f"Loaded {len(rows)} rows from {in_path}{' (limited by MDM_MAX_ROWS)' if max_rows else ''}")
    ...
    print(f"Done. Total: {len(results)} | Success: {succ} | Failed: {len(results)-succ} | Out: {out_path}")
